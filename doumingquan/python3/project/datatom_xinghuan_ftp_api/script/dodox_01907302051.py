...
#from GenerateWorkflowScript_ftp import init
...
class OperateDodox():
	...
	print(res_token)
	print(token_sql)
	print('this is user token',res_token[0][0])
	print('this is user token length',len(res_token))
        if len(res_token) == 1:
            token = res_token[0][0]
        else:
	...
	print("this is puyuan token",self.token)
        if len(resData) == 1:
            wfid = resData[0][1]
            status = resData[0][4]
            if status == 'up':
