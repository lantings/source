...
class OperDb():
    ...
    def source_params(self):
        try:
            self.conn =  MySQLdb.connect('172.27.148.98', 'detuo', 'DT@pt18cg', 'detuo_base', 3306,charset='utf8')
            print(self.conn)
            #self.conn = psycopg2.connect(database="postgres", user="stork",password="stork", host="172.27.148.53", port="14103")
            if self.conn:
