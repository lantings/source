...
def upload():
	file_number = upload_ftp('172.26.16.89',21,'detuoftp','detuoftp','/opt/datatom/ftpdata/','test12','huanbaoju')
	...
	if file_number[0]['code'] == 1:
		...
		path = "%s/huanbaoju/test12"%file_dir
		for root, dirs, files in os.walk(path):	
			if not os.path.isdir("%s/test12_split"%path):
				os.system("mkdir -p %s/test12_split"%path)
			for i in range(len(files)):
				os.system("split -l 30000000 %s/%s -d -a 10 %s/test12_split/%s"%(path,files[i],path,files[i]))
			break
	#新建txt以及orc表
		...
		if file_number[0]['filename'].split('_')[0] == 'csv': 
			...
			orc_table = "create table if not exists tdt.test3_%s(jhpt_update_time varchar(100) comment '数据生成时间',jhpt_time varchar(100) comment '数据时间')clustered by (JHPT_UPDATE_TIME) into 3 buckets stored as orc tblproperties('transactional'='true');"%addtime
		...
		#上传文件并加载到星环
		for file1,file2,file3 in os.walk("%s/test12_split"%path):
			i = 0
			for i in range(len(file3)):
				pri.info("data uploading %s..............."%file3[i])
				os.system("curl -i -X PUT \"http://172.26.41.32:32055/webhdfs/v1/tmp/detuo/huanbaoju_test12.txt?op=CREATE&guardian_access_token=6mKYTUMtterzxlYyMpv4-CEIJAZ5.TDH\"")
				#-C表示支持断点续传
				status,out=commands.getstatusoutput("curl -i -C - -X PUT -T %s/test12_split/%s \"http://172.26.41.32:32055/webhdfs/v1/tmp/detuo/huanbaoju_test12.txt?op=CREATE&data=true&guardian_access_token=6mKYTUMtterzxlYyMpv4-CEIJAZ5.TDH\" -H \"Content-Type:application/octet-stream\""%(path,file3[i]))
				time.sleep(5)
				print("status=",status)
				count = 1
				while (status != 0 and count <= 5):
					...
					status,out=commands.getstatusoutput("curl -i -C - -X PUT -T %s/test12_split/%s \"http://172.26.41.32:32055/webhdfs/v1/tmp/detuo/huanbaoju_test12.txt?op=CREATE&data=true&guardian_access_token=6mKYTUMtterzxlYyMpv4-CEIJAZ5.TDH\" -H \"Content-Type:application/octet-stream\""%(path,file3[i]))
					count = count + 1
				sqlStr = "load data inpath '/tmp/detuo/huanbaoju_test12.txt' into table tdt.test2_%s"%addtime
		...
		curs_truncate.close()
		conn.close()
		pri.ok("table test12_%s extracte succeed!"%addtime)
		os.system("rm -rf %s"%path)
		curs_txt.close()
	else:
		pri.error("ftp files is not exists")
