...
def upload():
	file_number = upload_ftp('172.26.16.89',21,'detuoftp','detuoftp','/opt/datatom/ftpdata/','base_test1','huanbaoju')
	...
	if file_number[0]['code'] == 1:
		...
		path = "%s/huanbaoju/base_test1"%file_dir
		print(path)
		if file_number[0]['filename'].split('_')[0] == 'xml':
			analyse_res = analyse_xml(path,path)
		if analyse_res == False:
			pri.error("xml解析错误！！！")
		for root, dirs, files in os.walk(path):	
			if not os.path.isdir("%s/base_test1_split"%path):
				os.system("mkdir -p %s/base_test1_split"%path)
			for i in range(len(files)):
				os.system("split -l 30000000 %s/%s -d -a 10 %s/base_test1_split/%s"%(path,files[i],path,files[i]))
			break
	#新建txt以及orc表
		...
		#上传文件并加载到星环
		for file1,file2,file3 in os.walk("%s/base_test1_split"%path):
			i = 0
			for i in range(len(file3)):
				pri.info("data uploading %s..............."%file3[i])
				os.system("curl -i -X PUT \"http://172.26.41.32:32055/webhdfs/v1/tmp/detuo/huanbaoju_base_test1.txt?op=CREATE&guardian_access_token=6mKYTUMtterzxlYyMpv4-CEIJAZ5.TDH\"")
				#-C表示支持断点续传
				status,out=commands.getstatusoutput("curl -i -C - -X PUT -T %s/base_test1_split/%s \"http://172.26.41.32:32055/webhdfs/v1/tmp/detuo/huanbaoju_base_test1.txt?op=CREATE&data=true&guardian_access_token=6mKYTUMtterzxlYyMpv4-CEIJAZ5.TDH\" -H \"Content-Type:application/octet-stream\""%(path,file3[i]))
				time.sleep(5)
				print("status=",status)
				count = 1
				while (status != 0 and count <= 5):
					...
					status,out=commands.getstatusoutput("curl -i -C - -X PUT -T %s/base_test1_split/%s \"http://172.26.41.32:32055/webhdfs/v1/tmp/detuo/huanbaoju_base_test1.txt?op=CREATE&data=true&guardian_access_token=6mKYTUMtterzxlYyMpv4-CEIJAZ5.TDH\" -H \"Content-Type:application/octet-stream\""%(path,file3[i]))
					count = count + 1
				sqlStr = "load data inpath '/tmp/detuo/huanbaoju_base_test1.txt' into table tdt.test2_%s"%addtime
		...
		curs_truncate.close()
		conn.close()
		pri.ok("table base_test1_%s extracte succeed!"%addtime)
