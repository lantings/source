...
def get_table_count(sourcetype,sourceaddress,sourceip,sourceport,sourcetable,sourceuser,sourcepasswd,xhtable,primary,addtimefield,scheduletype,pic_path):
	lists=[]
	if sourcetype == 'dameng':
		...
		print(wheredate)
		cursor.execute("select %s from %s where %s "%(pic_path,sourcetable,wheredate))
		count = cursor.fetchall()
		for i in range(len(count)):
			lists.append(str(count[i][0]).split("/")[-1])
...
if __name__ == '__main__':
	...
	x = get_table_count('mysql','jdbc:mysql://172.26.16.87:3306/test','172.26.16.87','3306','test.test1','root','datatom','tdt.test','id','jhpt_update_time','3','picture_path')
	print(x)
