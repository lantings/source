...
class OperateDodox():
    def __init__(self):
        danasip = "http://172.27.148.53"
        self.dber = OperDb()
        self.login_url =danasip+"/danastudio/login"
        self.create_url= "http://127.0.0.1/danastudio/ants/folder/create"
        self.upload_url = danasip+"/danastudio/dodox/filemanager/file/upload"
        self.workflow_url = danasip+"/danastudio/ants/workflow/new"
        self.script2wf_url = danasip+"/danastudio/ants/workflow/save"
        self.listall_url = danasip+"/danastudio/ants/folder/list"
        #self.search_url = danasip+"/danastudio/ants/develop/search"
        self.submit_url = danasip+"/danastudio/ants/workflow/submit"
        self.select_script_url = danasip+"/danastudio/ants/"
        self.list_job_url = danasip+"/danastudio/dodox/job/list"
        self.runnow_url = danasip+"/danastudio/dodox/job/runnow"
        self.get_log_url = danasip+"/danastudio/dodox/job/record/get"
        self.new_dev_url = danasip+"/danastudio/ants/develop/new/dev"
        self.down_url = danasip+"/danastudio/ants/workflow/down"
        self.del_wf_url = danasip+"/danastudio/ants/workflow/delete"
        self.stop_url = danasip+"/danastudio/dodox/job/stop"
        self.refresh_token_url = danasip+"/danastudio/refresh_token"
        self.lastjobstatus_url = danasip+"/danastudio/dodox/job/list"
        self.token = self.refresh_token("puyuan")
        if self.token.find('Bearer') == -1:
            logger.info('token is error, please check token_info and dana')
            return self.token()
    ...
    def refresh_token(self,user):
        token_sql = "select token from token_info where user='%s'"%user
	...
	print(token_sql)
        ...
        if res.find('expire') != -1:
            ...
            whereDict = {"1" : "1"}
            self.dber.update_sql('detuo_base.token_info',dataDict, whereDict)
            return token
        else:
            return res
    ...
    #创建整个工作流
    def create_all_nodes(self, dirName,filePathList,taskName,des,postfix,scriptType,node,extimes,scheduleDetail,scheduleType,recordTable,user):
	...
	print("++++++++++++",ssql)
	print(resData)
	self.token = self.refresh_token(user)
        if len(resData) == 1:
            wfid = resData[0][1]
            status = resData[0][4]
            if status == 'up':
	...
	print(filePathList)
	    ...
	    print('script upload to ds platform is success')
	    print(res_u)
            if res_u['code'] == 200:
	    ...
	    print("(((((((((((",res)
            if json.loads(res.encode('utf-8'))['code'] == 200:
                ...
                if json.loads(res.encode('utf-8'))['code'] == 200:
                    ssql = "select * from {recordTable} where workflowname = '{taskName}'".format(recordTable = recordTable,taskName=taskName)
                    resData = self.dber.select_sql(ssql)
                    print("99999999999999999999999",len(resData))
                    print("--------------",ssql)
                    if len(resData) == 0:
...
#直接上线一个已有的工作流
    def start_workflow(self,wid,extimes,scheduleDetail,scheduleType,node,taskName,recordTable):
        ssql = "select * from {recordTable} where wfid = '{wfid}'".format(recordTable = recordTable,wfid=wid)
        resData = self.dber.select_sql(ssql)
        if len(resData) == 1:
            res = self.submit_workflow(node,extimes,wid,scheduleDetail,scheduleType)
            if json.loads(res.encode('utf-8'))['code'] == 200:
                res = {"code":200,"result":"工作流上线成功"}
                res = json.dumps(res)
            else:
                res = {"code":10003,"result":"工作流上线失败"}
                res = json.dumps(res)
            return res
        else:
            res = {"code":10003,"result":"此工作流不存在，请上传新工作流"}
            res = json.dumps(res)
            return res

#直接下线一个已有的工作流
    def stop_workflow(self,wid,recordTable):
        ssql = "select * from {recordTable} where wfid = '{wfid}'".format(recordTable = recordTable,wfid=wid)
        resData = self.dber.select_sql(ssql)
        if len(resData) == 1:
            res = self.down_workflow(wid)
            if json.loads(res.encode('utf-8'))['code'] == 200:
                res = {"code":200,"result":"工作流下线成功"}
                res = json.dumps(res)
            else:
                res = {"code":10003,"result":"工作流下线失败"}
                res = json.dumps(res)
            return res
        else:
            res = {"code":10003,"result":"此工作流不存在，"}
            res = json.dumps(res)
            return res
    ...
    def job_runnow(self,wfid,userid):
        ...
        while 1 <= count <=5:
	    time.sleep(10)
            res1 = self.fail_runnew(wfid, userid)
	    print("======进行第%s次循环======"%count)
	    count += 1
	#print(res)
        return res

    #检测上次运行结果,失败重跑
    def fail_runnew(self,wfid,userid):
	print('======进行判断=========')
        #检查任务运行状态
        res = self.get_last_job_status(wfid, userid)
        val = json.loads(res.encode('utf-8'))
        if val['code'] == 200:
            lastrunstatus = val['status']
            if lastrunstatus == 'FAIL':
                #运行任务
		print("=======last_status ： FAIL！！!===========")
                res = self.job_run(wfid, userid)
                # count = count+1
                return res
        else:
	    print("======code != 200")
    ...
    #获取任务上次的结果判断
    def get_last_job_status(self,wfid,userid):
	print("=======获得上次结果：=========")
        res = self.get_last_job(userid)
        val = json.loads(res.encode('utf-8'))
        if val['code'] == 200:
	   #print("======code 200=============")
            allInfo = val['result']
			...
			#print("|||||||||")
                        ...
                        print("=======%s,%s============"%(wfid,lastrunstatus))
                res = {"code": 200,"status":lastrunstatus}
                res = json.dumps(res)
                return res
            else:
	...
	print(userid)
        res = self.request_func(data,self.lastjobstatus_url)
	#print("====res======",res)
        return res
 

    def job_run(self, wfid, userid):
        data = {
            "id": wfid,
            "userid": userid
        }
