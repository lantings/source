...
def action_datax():
	ScheduleType = '1'
	if ScheduleType == '1':
		#os.system("python /opt/datatom/datax/bin/datax.py -p \"-Dtimecolumn=%s\" /opt/datatom/dana_appendix_api/execute_all/huanbaoju_appendix_16_88_syrk_t_zp_edz_2.json > /opt/datatom/dana_appendix_api/log/${exec_datax_log}/huanbaoju_appendix_16_88_syrk_t_zp_edz_2.log"%addtime)
		os.system("python /opt/datatom/datax/bin/datax.py -p \"-Dtimecolumn=%s\" /opt/datatom/dana_appendix_api/execute_all/huanbaoju_appendix_16_88_syrk_t_zp_edz_2.json > /opt/datatom/dana_appendix_api/log/huanbaoju_appendix_16_88_syrk_t_zp_edz_2.log"%addtime)
		#sed_log('huanbaoju_appendix_16_88_syrk_t_zp_edz_2')
		operate_tables('/opt/datatom/dana_appendix_api/log/huanbaoju_appendix_16_88_syrk_t_zp_edz_2.log', 'gonganju_txt.T_ZP_EDZ_2_%s'%day_time, 'SYRK.T_ZP_EDZ_2','detuo_base.job_run_info')
		pri.ok("table SYRK.T_ZP_EDZ_2 data extraction txt succeed，uploading to inceptor!")
	elif ScheduleType == '3':
		#os.system("python /opt/datatom/datax/bin/datax.py -p \"-Dtimecolumn=%s\" /opt/datatom/dana_appendix_api/execute_all/huanbaoju_appendix_16_88_syrk_t_zp_edz_2.json > /opt/datatom/dana_appendix_api/log/${exec_datax_log}/huanbaoju_appendix_16_88_syrk_t_zp_edz_2.log"%addtime)
		os.system("python /opt/datatom/datax/bin/datax.py -p \"-Dtimecolumn=%s\" /opt/datatom/dana_appendix_api/execute_all/huanbaoju_appendix_16_88_syrk_t_zp_edz_2.json > /opt/datatom/dana_appendix_api/log/huanbaoju_appendix_16_88_syrk_t_zp_edz_2.log"%addtime)
		#sed_log('huanbaoju_appendix_16_88_syrk_t_zp_edz_2')
		operate_tables('/opt/datatom/dana_appendix_api/log/huanbaoju_appendix_16_88_syrk_t_zp_edz_2.log', 'gonganju_txt.T_ZP_EDZ_2_%s'%month_time, 'SYRK.T_ZP_EDZ_2','detuo_base.job_run_info')
		pri.ok("table SYRK.T_ZP_EDZ_2 data extraction txt succeed，uploading to inceptor!")
	else:
...
def upload():
	...
	appendix_path = "%s/huanbaoju/SYRK_T_ZP_EDZ_2"%appendix_dir
	#数据切割，防止一次性数据量过大
	...
	if ScheduleType == '1':
		...
		#源库存的是附件路径
			...
			res = upload_ftp('172.26.16.89', 21, 'detuoftp', 'detuoftp', '/opt/datatom/ftp/', 'huanbaoju','T_ZP_EDZ_2')
			if res == True:
				#新建txt以及orc表
				txt_table = "create table if not exists gonganju_txt.T_ZP_EDZ_2_%s(ID string comment 'ID',ZJHM string comment '证件号码',ZP blob comment '图片',ZPRQ string comment '照片日期',SJGXSJ string comment '数据共享时间',PHOTO_NO string comment '',JLRKSJ string comment '',RID string comment '')ROW FORMAT DELIMITED  FIELDS TERMINATED BY '\u0001';"%day_time
				orc_table = "create table if not exists gonganju_ods.T_ZP_EDZ_2_%s(ID string comment 'ID',ZJHM string comment '证件号码',ZP blob comment '图片',ZPRQ string comment '照片日期',SJGXSJ string comment '数据共享时间',PHOTO_NO string comment '',JLRKSJ string comment '',RID string comment '') stored as hyperdrive;"%day_time
				...
				pri.info("---------------------create xinghuan table is success----------------")
				#新建照片表
				txt_zp_table = "create table if not exists gonganju_txt.T_ZP_EDZ_2_zp (zpid string, zpsj timestamp, zp blob) ROW FORMAT DELIMITED  FIELDS TERMINATED BY '\u0001';"
				txt_zp_table = "create table if not exists gonganju_ods.T_ZP_EDZ_2_zp (zpid string, zpsj timestamp, zp blob) stored as hyperdrive;"
				curs_txt.execute(txt_table)
				curs_txt.execute(orc_table)
				...
				for file1,file2,file3 in os.walk("%s/huanbaoju/SYRK_T_ZP_EDZ_2"%appendix_path):
					i=0
					for i in range(len(file3)):
						pri.info("data uploading %s..............."%file3[i])
						os.system("curl -i -X PUT \"http://172.27.148.14:31756/webhdfs/v1/tmp/detuo/huanbaoju_SYRK_T_ZP_EDZ_2.txt?op=CREATE&guardian_access_token=da30EEF4lIxgm6QQKwpg-M03NIQ5.TDH\"")
						#-C表示支持断点续传
						status,out=commands.getstatusoutput("curl -i -C - -X PUT -T %s/huanbaoju/SYRK_T_ZP_EDZ_2/%s \"http://172.27.148.14:31756/webhdfs/v1/tmp/detuo/huanbaoju_SYRK_T_ZP_EDZ_2.txt?op=CREATE&data=true&guardian_access_token=da30EEF4lIxgm6QQKwpg-M03NIQ5.TDH\" -H \"Content-Type:application/octet-stream\""%(appendix_path,file3[i]))
						time.sleep(5)
						print("status=",status)
						count = 1
						while (status != 0 and count <= 5):
							...
							status,out=commands.getstatusoutput("curl -i -C - -X PUT -T %s/huanbaoju/SYRK_T_ZP_EDZ_2/%s \"http://172.27.148.14:31756/webhdfs/v1/tmp/detuo/huanbaoju_SYRK_T_ZP_EDZ_2.txt?op=CREATE&data=true&guardian_access_token=da30EEF4lIxgm6QQKwpg-M03NIQ5.TDH\" -H \"Content-Type:application/octet-stream\""%(appendix_path,file3[i]))
							count = count + 1
						sqlStr_zp = "load data inpath '/tmp/detuo/huanbaoju_SYRK_T_ZP_EDZ_2.txt' into table gonganju_txt.T_ZP_EDZ_2_zp"
				...
				if Execrate == '2':
					...
					pri.info("incremental data extraction per day!")
				sql_orc = "insert into gonganju_ods.T_ZP_EDZ_2_%s (ID,ZJHM,ZP,ZPRQ,SJGXSJ,PHOTO_NO,JLRKSJ,RID) select ID,ZJHM,ZP,ZPRQ,SJGXSJ,PHOTO_NO,JLRKSJ,RID  from gonganju_txt.T_ZP_EDZ_2_%s"%(day_time,day_time)
				sql_orc_zp = "insert into gonganju_ods.T_ZP_EDZ_2_zp  select * from gonganju_txt.T_ZP_EDZ_2_zp"
				curs_insertorc.execute(sql_orc)
				curs_insertorc.execute(sql_orc_zp)
				pri.ok("insert orc table from txt table succeed!")	
				curs_truncate = conn.cursor()
				sql_truncate = "truncate table gonganju_txt.T_ZP_EDZ_2_%s"%day_time
				sql_truncate_zp = "truncate table gonganju_txt.T_ZP_EDZ_2_zp"
				curs_truncate.execute(sql_truncate)
				curs_truncate.execute(sql_truncate_zp)
				pri.ok("truncate txt table gonganju_txt.T_ZP_EDZ_2_%s and zp succeed!"%day_time)
				curs_load.close()
				curs_insertorc.close()
				curs_truncate.close()
				conn.close()
    ...
    #数据归集类型为每月
		...
		if Type == '1':
			...
			orc_table = "create table if not exists gonganju_ods.T_ZP_EDZ_2_%s(ID string comment 'ID',ZJHM string comment '证件号码',ZP blob comment '图片',ZPRQ string comment '照片日期',SJGXSJ string comment '数据共享时间',PHOTO_NO string comment '',JLRKSJ string comment '',RID string comment '')clustered by (ID) into ${buckets} buckets stored as orc tblproperties('transactional'='true');"%month_time
			print("xinghuan_txt create table sql:",txt_table)
			print("xinghuan_orc create table sql:",orc_table)
			curs_txt.execute(txt_table)
			curs_txt.execute(orc_table)
		...
		#源库存的是附件路径
			...
			if res == True:
				#新建txt以及orc表
				txt_table = "create table if not exists gonganju_txt.T_ZP_EDZ_2_%s(ID string comment 'ID',ZJHM string comment '证件号码',ZP blob comment '图片',ZPRQ string comment '照片日期',SJGXSJ string comment '数据共享时间',PHOTO_NO string comment '',JLRKSJ string comment '',RID string comment '')ROW FORMAT DELIMITED  FIELDS TERMINATED BY '\u0001';"%month_time
				orc_table = "create table if not exists gonganju_ods.T_ZP_EDZ_2_%s(ID string comment 'ID',ZJHM string comment '证件号码',ZP blob comment '图片',ZPRQ string comment '照片日期',SJGXSJ string comment '数据共享时间',PHOTO_NO string comment '',JLRKSJ string comment '',RID string comment '') stored as hyperdrive;"%month_time
				print("xinghuan_txt create table sql:",txt_table)
				print("xinghuan_orc create table sql:",orc_table)
				curs_txt.execute(txt_table)
				curs_txt.execute(orc_table)
				...
				pri.info("---------------------create xinghuan_zp table is success----------------")
				#上传文件并加载到星环
				for file1,file2,file3 in os.walk("%s/huanbaoju/SYRK_T_ZP_EDZ_2"%path):
					i=0
					for i in range(len(file3)):
						pri.info("data uploading %s..............."%file3[i])
						os.system("curl -i -X PUT \"http://172.27.148.14:31756/webhdfs/v1/tmp/detuo/huanbaoju_SYRK_T_ZP_EDZ_2.txt?op=CREATE&guardian_access_token=da30EEF4lIxgm6QQKwpg-M03NIQ5.TDH\"")
						#-C表示支持断点续传
						status,out = commands.getstatusoutput("curl -i -C - -X PUT -T %s/huanbaoju/SYRK_T_ZP_EDZ_2/%s \"http://172.27.148.14:31756/webhdfs/v1/tmp/detuo/huanbaoju_SYRK_T_ZP_EDZ_2.txt?op=CREATE&data=true&guardian_access_token=da30EEF4lIxgm6QQKwpg-M03NIQ5.TDH\" -H \"Content-Type:application/octet-stream\""%(path,file3[i]))
						time.sleep(5)
						print("status=",status)
						count = 1
						while (status != 0 and count <= 5):
