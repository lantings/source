...
def source_params(UnitId,Dbname):
	...
	try:
		...
		cursor = conn.cursor()
		cursor.execute("select * from source_info where unitid = '%s' and sourcedbname = '%s'"%(UnitId,Dbname))
...
def init(path_name):
	...
	if os.path.exists(table_conf):
		...
		return UnitName,MappingList,SourceTableName,XinghuanAddress,XinghuanUsername,XinghuanPassword,PrimaryKey,HttpfsAddress,XinghuanTableName,XinghuanOrcTableName,Exectimes,AddTimefield,ColumnType,ScheduleType,TargetList,Execrate
	else:
...
def datax_update(UnitId,path_name,Dbname):
	UnitName,MappingList,SourceTableName,XinghuanAddress,XinghuanUsername,XinghuanPassword,PrimaryKey,HttpfsAddress,XinghuanTableName,XinghuanOrcTableName,Exectimes,AddTimefield,ColumnType,ScheduleType,TargetList,Execrate = init(path_name)
		...
		if Exectimes == '-1' or Exectimes == '1':
			if databasetype == 'mysql':
				if ScheduleType == '1':
					...
					if ColumnType == 'varchar':
						wherevarchar = "%s >= DATE_FORMAT(DATA_SUB(DATE_SUB(STR_TO_DATE(${timecolumn},'%%Y%%m%%d'),INTERVAL 1 MONTH),INTERVAL 1 DAY),'%%Y%%m%%d') and %s < DATE_FORMAT(DATE_SUB(	STR_TO_DATE(${timecolumn},'%%Y%%m%%d'),INTERVAL 1 DAY),'%%Y%%m%%d') "%(AddTimefield,AddTimefield)
						where = wherevarchar
					elif ColumnType == 'date' or ColumnType == 'timestamp' or ColumnType == 'datetime':
...
def datax_all_file(UnitId,path_name,Dbname):
	...
	if Execrate == '2':
		...
		if databasetype == 'mysql':
			if ScheduleType == '1':
				if ColumnType == 'varchar':
					wherevarchar = "%s < DATE_FORMAT(DATE_SUB(STR_TO_DATE(${timecolumn},'%%Y%%m%%d'),INTERVAL 2 DAY),'%%Y%%m%%d')"%AddTimefield
					where = wherevarchar
				elif ColumnType == 'date' or ColumnType == 'timestamp' or ColumnType == 'datetime':
					wheredate = "%s < DATE_SUB(STR_TO_DATE(${timecolumn},'%%Y%%m%%d'),INTERVAL 2 DAY)"%AddTimefield
					where = wheredate
				else:
					pri.error("the parameter ColumnType is not exists or type error!")
			elif ScheduleType == '2':
				if ColumnType == 'varchar':
					wherevarchar = "%s < DATE_FORMAT(DATE_SUB(STR_TO_DATE(${timecolumn},'%%Y%%m%%d'),INTERVAL 8 DAY),'%%Y%%m%%d')"%AddTimefield
					where = wherevarchar
				elif ColumnType == 'date' or  ColumnType =='timestamp' or ColumnType == 'datetime':
					wheredate = "%s < DATE_SUB(STR_TO_DATE(${timecolumn},'%%Y%%m%%d'),INTERVAL 8 DAY)"%AddTimefield
					where = wheredate
				else:
					pri.error("the parameter ColumnType is not exists or type error!")
			elif ScheduleType == '3':
				if ColumnType == 'varchar':
					...
					wheredate = "%s < DATA_SUB(DATE_SUB(STR_TO_DATE(${timecolumn},'%%Y%%m%%d'),INTERVAL 1 MONTH),INTERVAL 1 DAY)"%AddTimefield
					where = wheredate
				else:
					pri.error("the parameter ColumnType is not exists or type error!")
			else:
		...
		if databasetype == 'oracle' or databasetype == 'dameng':
			if ScheduleType == '1':
				if ColumnType == 'varchar2':
					wherevarchar2 = "%s < to_char(to_date(${timecolumn},'yyyymmdd') - INTERVAL '2' DAY , 'yyyymmdd')"%AddTimefield
					where = wherevarchar2 
				elif ColumnType == 'date'  or ColumnType == 'timestamp' or ColumnType == 'datetime':
					wheredate = "%s < to_date(${timecolumn},'yyyymmdd') - INTERVAL '2' DAY"%AddTimefield
					where = wheredate
				else:
					pri.error("the parameter ColumnType is not exists or type error!")
			elif ScheduleType == '2':
				if ColumnType == 'varchar2':
					wherevarchar2 = "%s < to_char(to_date(${timecolumn},'yyyymmdd') - INTERVAL '8' DAY , 'yyyymmdd')"%AddTimefield
					where = wherevarchar2 
				elif ColumnType == 'date'  or ColumnType == 'timestamp' or ColumnType == 'datetime':
					wheredate = "%s < to_date(${timecolumn},'yyyymmdd') - INTERVAL '8' DAY"%AddTimefield
					where = wheredate
				else:
					pri.error("the parameter ColumnType is not exists or type error!")
			elif ScheduleType == '3':
				if ColumnType == 'varchar2':
					...
					wheredate = "%s < to_date(${timecolumn},'yyyymmdd') - INTERVAL '1' MONTH - INTERVAL 1 DAY"%AddTimefield
					where = wheredate
				else:
					pri.error("the parameter ColumnType is not exists or type error!")
			else:
...
if __name__ == '__main__':
	unitname,databasetype,sourceip,sourceport,sourcedbname,sourceaddress,sourceusername,sourcepassword = source_params('765123','test')
	UnitName,MappingList,SourceTableName,XinghuanAddress,XinghuanUsername,XinghuanPassword,PrimaryKey,HttpfsAddress,XinghuanTableName,XinghuanOrcTableName,Exectimes,AddTimefield,ColumnType,ScheduleType,TargetList,Execrate = init('huanbaoju_16_87_test_logs_info')
	source_tbname = SourceTableName.split('.')[1]
	source_dbname = SourceTableName.split('.')[0]
	xinghuan_tbname = XinghuanTableName.split('.')[1]
	xinghuan_dbname = XinghuanTableName.split('.')[0]
